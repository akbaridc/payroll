...
export default function TransPayrollEdit() {
  ...
  const insertPayrollDetail = async () => {
    try {
      const payload = {
        trans_payroll_id,
        pengguna_username: user().name,
      };
      await axios.post(`/api/ProsesHitungPayrollAsliKeTemp`, payload);
    } catch {}
  };
  ...
  const handleSave = form.handleSubmit(async () => {
    ...
    try {
      ...
      const res = await axios.put(`/api/TransPayroll/${trans_payroll_id}`, payload);
      if (res.status === 200) {
